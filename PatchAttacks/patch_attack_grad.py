...
def MultiPatchGDAttack(model, img, label, loss=nn.CrossEntropyLoss(), iterations=40, device=torch.device('cuda:0'), max_num_patches=100, *args, **kwargs):
    ...
    for i in range(0, sx, patch_size):
        for j in range(0, sy, patch_size):
            grad_mags[(i,j)] = torch.norm(grad_val[0][:,:,i:i+patch_size, j:j+patch_size], p="fro")
    ...
    for k in range(1, max_num_patches+1):
        ...
        for i in range(iterations):
            ...
            for p in range(k):
                ...
                img[:,:,p_x:p_x+patch_size, p_y:p_y+patch_size] += lr * grad_val[0][:,:,p_x:p_x+patch_size, p_y:p_y+patch_size]
            with torch.no_grad():
                pred2 = model(img)
...
def build_parser():
    ...
    parser.add_argument('-mp', '--max_patches', help='Max number of patches allowed to be perturbed', type=int, default=20)
    parser.add_argument('-ni', '--num_images', help='Number of images to be tested', default=100, type=int)
    return parser
...
if __name__ == '__main__':
    ...
    for idx, (img, label) in enumerate(test_dl):
        ...
        bs, ch, sx, sy = img.shape
        label = label.to(device)
        succ, img, attack_img, num_patches = MultiPatchGDAttack(model, img, label, loss=nn.CrossEntropyLoss(), iterations=args.iter, device=device, max_num_patches=args.max_patches)
