...
def get_data_loader(batch_size=16, mode='train'):
    transforms = Compose([Resize(384), ToTensor(), Normalize(mean=(0.5,0.5, 0.5), std=(0.5, 0.5, 0.5))])
    if mode == 'train':
        ds = CIFAR10(root='./datasets/train/', download=True, train=True, transform=transforms)
    else:        
        ds = CIFAR10(root='./datasets/test/', download=True, train=False, transform=transforms)
...
def validate(model, device, ds_loader, criterion):
    ...
    with torch.no_grad():
        ...
        val_loss = val_loss/len(ds_loader)
    return val_loss, acc
...
def train(args):
    ...
    train_dl = get_data_loader(batch_size=args.batchsize, mode='train')
    val_dl = get_data_loader(batch_size=args.batchsize, mode='val')
    test_dl = get_data_loader(batch_size=args.batchsize, mode='test')
